...
func (r *blogPostResolver) Versions(ctx context.Context, obj *BlogPost) ([]*BlogPostVersion, error) {
	// Parse the post ID directly from the object — no DB roundtrip needed
	postIDUint64, err := strconv.ParseUint(obj.ID, 10, 64)
	if err != nil {
		return []*BlogPostVersion{}, nil
	}
	postID := uint(postIDUint64)

	// 使用 DataLoader 获取版本历史
	loader := GetDataLoaderFromContext(ctx)
	var versions []*models.BlogPostVersion

	if loader != nil {
		versions, err = loader.LoadVersions(ctx, uint(postID))
		if err != nil {
			return []*BlogPostVersion{}, nil
		}
	} else {
		// Fallback
		var v []models.BlogPostVersion
...
func (r *mutationResolver) Register(ctx context.Context, input RegisterInput) (*AuthPayload, error) {
	...
	user, token, refreshToken, err := authService.Register(registerInput)
	if err != nil {
		return nil, err
	}
...
func (r *mutationResolver) UploadImage(ctx context.Context, file graphql.Upload) (*ImageUploadResponse, error) {
	...
	// 使用文件服务上传图片
	fileService := services.NewFileService(r.Resolver.DB)
...
func (r *mutationResolver) RebuildSearchIndex(ctx context.Context) (*GeneralResponse, error) {
	...
	if err := r.DB.Transaction(func(tx *gorm.DB) error {
		if err := tx.Exec("DELETE FROM blog_post_fts").Error; err != nil {
			return fmt.Errorf("清空FTS表失败: %w", err)
		}
		...
		for _, post := range posts {
			if err := tx.Exec(insertSQL, post.ID, post.Title, post.Content, post.Tags, post.Categories).Error; err != nil {
				return fmt.Errorf("索引文章 %d 失败: %w", post.ID, err)
			}
		}
		return nil
	}); err != nil {
		logger.Errorw("搜索索引重建失败", "error", err)
		return &GeneralResponse{
			Success: false,
			Message: strPtr(fmt.Sprintf("搜索索引重建失败: %v", err)),
			Code:    strPtr("REBUILD_INDEX_FAILED"),
		}, nil
	}

	if searchService != nil && !searchService.RefreshFTSSupport() {
		logger.Errorw("搜索索引重建后 FTS 仍不可用")
		return &GeneralResponse{
			Success: false,
			Message: strPtr("搜索索引重建失败: FTS 索引不可用"),
			Code:    strPtr("REBUILD_INDEX_FAILED"),
		}, nil
	}
	...
	logger.Infow("搜索索引重建完成", "total", len(posts), "success", len(posts))

	return &GeneralResponse{
		Success: true,
		Message: strPtr(fmt.Sprintf("搜索索引重建完成，共索引 %d/%d 篇文章", len(posts), len(posts))),
...
func (r *queryResolver) Comments(ctx context.Context, blogPostID *string, limit *int, offset *int, filter *CommentFilterInput, sort *CommentSortInput) (*CommentResult, error) {
	...
	dbQuery := r.Resolver.DB.Model(&models.BlogPostComment{}).
		...
		Preload("Replies", func(db *gorm.DB) *gorm.DB {
			return db.Preload("User").Preload("BlogPost")
		})
