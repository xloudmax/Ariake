...
export const LiquidSwitch: FC<LiquidSwitchProps> = React.memo(({
  ...
  // Thumb motion values (must be at top-level, not inline in JSX)
  const thumbX = useTransform(() => xRatio.get() * TRAVEL)
  const thumbBgColor = useTransform(backgroundOpacity, (op) => `rgba(255, 255, 255, ${op})`)
  const thumbBoxShadow = useTransform(() => {
